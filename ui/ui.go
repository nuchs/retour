...
type Model struct {
	records     []db.Record // All available records
	filtered    []db.Record // Records after filtering
...
func (m Model) Records() []db.Record {
	return m.filtered
}
...
func New(records []db.Record) Model {
	return Model{
		records:    records,
...
func (m Model) Selected() (db.Record, bool) {
...
func formatRecord(r db.Record) string {
	status := "✓"
	if r.ExitStatus != 0 {
		status = "✗"
	}
